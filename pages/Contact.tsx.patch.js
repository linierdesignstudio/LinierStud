...
`;
code = code.replace(/onClick=\{\(e\) => \{[\s\S]*?e\.currentTarget\.href = [^;]+;\s*\}\}/, onClickReplacement.trim());

// 3. Add error messages to fields
// Name
code = code.replace(
  '{t.contact.form.name}</label>',
  '{t.contact.form.name}</label>\n                            {errors.name && <span className="absolute -bottom-5 left-0 text-xs text-red-500 font-bold uppercase tracking-widest">{t.common?.required || "Wajib diisi"}</span>}'
);
// Email
code = code.replace(
  '{t.contact.form.email}</label>',
  '{t.contact.form.email}</label>\n                            {errors.email && <span className="absolute -bottom-5 left-0 text-xs text-red-500 font-bold uppercase tracking-widest">{t.common?.required || "Wajib diisi"}</span>}'
);
// Phone
code = code.replace(
  '{t.contact.form.phone}</label>',
  '{t.contact.form.phone}</label>\n                            {errors.phone && <span className="absolute -bottom-5 left-0 text-xs text-red-500 font-bold uppercase tracking-widest">{t.common?.required || "Wajib diisi"}</span>}'
);
// Message
code = code.replace(
  '{t.contact.form.message}</label>',
  '{t.contact.form.message}</label>\n                        {errors.message && <span className="absolute -bottom-5 left-0 text-xs text-red-500 font-bold uppercase tracking-widest">{t.common?.required || "Wajib diisi"}</span>}'
);
